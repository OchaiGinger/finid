...
import { Shield, Activity, Lock, TrendingUp, Sparkles, ChevronRight } from 'lucide-react';
...
export function SleekScoreSection() {
  ...
  // Subtle parallax for floating items
  ...
  return (
    <section ref={containerRef} className="relative bg-[#050505] py-32 overflow-hidden text-white">
      
      {/* GITHUB-STYLE FLOATING BACKGROUND ITEMS */}
      <div className="absolute inset-0 pointer-events-none">
        <motion.div style={{ y: y1 }} className="absolute top-[10%] left-[15%] w-64 h-64 bg-[#B22222]/10 rounded-full blur-[120px]" />
        <motion.div style={{ y: y2 }} className="absolute bottom-[10%] right-[10%] w-96 h-96 bg-[#B22222]/5 rounded-full blur-[150px]" />
          ...
          <div className="lg:col-span-5 flex flex-col items-center justify-center">
            <div className="relative group">
              ...
              <div className="w-64 h-64 md:w-80 md:h-80 rounded-full bg-black border border-white/5 flex flex-col items-center justify-center relative shadow-[0_0_50px_rgba(178,34,34,0.1)]">
                ...
                <span className="text-[10px] uppercase tracking-[0.5em] text-white/30 mt-2">Trust Score</span>
              </div>
            </div>
          </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-6">
                      <div className={cn(
                        ...
                        activeIndex === i ? "bg-[#B22222] text-white" : "bg-white/5 text-white/40"
                      )}>
                        <item.icon size={20} />
                      </div>
                      <div>
                    ...
                    {activeIndex === i && (
                      ...
                      >
                        <span className="text-3xl font-black text-[#B22222]">{item.score}%</span>
                        <p className="text-[8px] uppercase tracking-widest text-white/30">Verified</p>
                      </motion.div>
                    )}
                  </div>

                  {/* Sleek Progress Loader underneath */}
                  {activeIndex === i && (
                    <motion.div 
                      layoutId="loader"
