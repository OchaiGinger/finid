...
export function ApiPreview() {
  ...
  const codeExamples = {
    curl: `curl -X GET "https://api.Agnexi.io/v1/identity/842" \\
  -H "Authorization: Bearer YOUR_API_KEY" \\
  -H "Content-Type: application/json"`,
    node: `const Agnexi = require('@Agnexi/sdk');

const client = Agnexi.init('YOUR_API_KEY');
const score = await client.identity.getScore('user_id_123');

console.log(score.trust_rating); // 842`,
    python: `import Agnexi

client = Agnexi.Client(api_key='YOUR_API_KEY')
      ...
      <div className="container mx-auto px-6">
        <div className="max-w-3xl mx-auto text-center mb-16">
          ...
          <p className="text-muted-foreground text-lg">
            Integrate global trust data into your application in minutes.
            Simple, powerful, and secure by design.
          </p>
        </div>
            ...
            <Tabs defaultValue="curl" className="w-full">
              <div className="bg-[#12151C] px-6 pt-4 border-b border-white/10">
                <TabsList className="bg-white/5 border-none p-1">
                  <TabsTrigger
                    value="curl"
                    className="data-[state=active]:bg-primary data-[state=active]:text-white rounded-lg px-4 font-code text-xs"
                  >
                    cURL
                  </TabsTrigger>
                  <TabsTrigger
                    ...
                    <code>
                      {code.split("\n").map((line, i) => (
                        <div key={i} className="flex gap-4">
                          <span className="text-white/20 select-none w-4">
                            {i + 1}
                          </span>
                          <span>{line}</span>
                        </div>
